...
const relevantProducts = productData.products.filter(product => 
...
function initializeFilters() {
  ...
  const producerDropdown = document.querySelector('[data-filter="producer"] .dropdown-menu');
  ...
  const locationDropdown = document.querySelector('[data-filter="location"] .dropdown-menu');
  ...
  const availabilityDropdown = document.querySelector('[data-filter="availability"] .dropdown-menu');
  ...
  const formatDropdown = document.querySelector('[data-filter="format"] .dropdown-menu');
  ...
  const harvestDropdown = document.querySelector('[data-filter="harvest_date"] .dropdown-menu');
  const uniqueDates = [...new Set(relevantProducts.map(p => p.harvest_date))].sort();
  ...
  const deliveryDropdown = document.querySelector('[data-filter="delivery"] .dropdown-menu');
...
function applyFilters() {
  filteredProducts = relevantProducts.filter(product => {
    return (
      (!currentFilters.producer || product.producer === currentFilters.producer) &&
      (!currentFilters.location || product.location === currentFilters.location) &&
      (!currentFilters.availability || product.availability === currentFilters.availability) &&
      (!currentFilters.format || product.format === currentFilters.format) &&
      (!currentFilters.harvest_date || product.harvest_date === currentFilters.harvest_date) &&
...
function applySort() {
  ...
  switch (currentSort) {
    ...
    case 'name-desc':
      ...
      break;
    case 'harvest-desc':
      sortedProducts.sort((a, b) => new Date(b.harvest_date) - new Date(a.harvest_date));
      break;
    case 'availability':
